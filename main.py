...
class SerialTerminal:
    def __init__(self, root):
        ...
        # Test EFFECTS button
        ttk.Button(sel_frame, text="Test EFFECTS", command=self.open_effects_tester).grid(row=4, column=0, columnspan=4, pady=5)
    ...
    def open_playlist_builder(self):
        ...
        ttk.Label(btn_frame, text="Duration (ms):").pack(side=tk.LEFT)
        self.play_dur_var = tk.StringVar(value="0")
        ttk.Entry(btn_frame, textvariable=self.play_dur_var, width=10).pack(side=tk.LEFT, padx=5)
    ...
    def open_effects_tester(self):
        ...
        notebook = ttk.Notebook(self.effects_window)
        ...
        ttk.Label(color_frame, text="Red:").pack(pady=(10,0))
        ...
        ttk.Label(color_frame, text="Green:").pack(pady=(10,0))
        ...
        ttk.Label(color_frame, text="Blue:").pack(pady=(10,0))
        ...
        ttk.Button(color_frame, text="Send", command=self.send_color).pack(pady=20)
        ...
        # Initial button
        self.initial_btn = ttk.Button(marquee_frame, text="Add Color", command=self.show_color_selectors)
        self.initial_btn.pack(expand=True, pady=20)
        
        # Color selectors frame
        self.color_select_frame = ttk.Frame(marquee_frame)
        ttk.Label(self.color_select_frame, text="Red:").pack(pady=(10,0))
        self.marquee_red_var = tk.DoubleVar(value=0)
        ttk.Scale(self.color_select_frame, from_=0, to=100, variable=self.marquee_red_var, orient=tk.HORIZONTAL, length=300, command=self.update_preview).pack(pady=5)
        
        ttk.Label(self.color_select_frame, text="Green:").pack(pady=(10,0))
        self.marquee_green_var = tk.DoubleVar(value=0)
        ttk.Scale(self.color_select_frame, from_=0, to=100, variable=self.marquee_green_var, orient=tk.HORIZONTAL, length=300, command=self.update_preview).pack(pady=5)
        
        ttk.Label(self.color_select_frame, text="Blue:").pack(pady=(10,0))
        self.marquee_blue_var = tk.DoubleVar(value=0)
        ttk.Scale(self.color_select_frame, from_=0, to=100, variable=self.marquee_blue_var, orient=tk.HORIZONTAL, length=300, command=self.update_preview).pack(pady=5)
        ...
        ttk.Button(self.color_select_frame, text="Select", command=self.show_size_slider).pack(pady=10)
        self.color_select_frame.pack_forget()
        
        # Size slider frame
        self.size_frame = ttk.Frame(marquee_frame)
        ttk.Label(self.size_frame, text="Size (ft):").pack(pady=(10,0))
        self.size_var = tk.DoubleVar(value=0.25)
        ttk.Scale(self.size_frame, from_=0.25, to=100, variable=self.size_var, orient=tk.HORIZONTAL, length=300, command=self.update_size_label).pack(pady=5)
        self.size_label = ttk.Label(self.size_frame, text=f"{self.size_var.get():.2f} ft")
        self.size_label.pack(pady=5)
        ttk.Button(self.size_frame, text="Save", command=self.add_color_block).pack(pady=10)
        ...
        self.timeline_canvas = tk.Canvas(self.timeline_frame, height=60, bg="#333333", width=500)
        ...
        self.plus_btn = ttk.Button(self.plus_frame, text="+", width=3, command=self.on_plus_click)
        self.plus_btn.pack()
        ...
        # Advanced properties
        self.adv_frame = ttk.LabelFrame(marquee_frame, text="Advanced Properties")
        self.adv_frame.pack(fill=tk.X, pady=10)
        
        ttk.Label(self.adv_frame, text="Marquee Speed:").pack(anchor="w")
        self.marquee_speed_var = tk.DoubleVar(value=0)
        ttk.Scale(self.adv_frame, from_=-100, to=100, variable=self.marquee_speed_var, orient=tk.HORIZONTAL, length=300, command=self.update_marquee_speed_label).pack(pady=5)
        self.marquee_speed_label = ttk.Label(self.adv_frame, text=f"{self.marquee_speed_var.get():.2f}")
        self.marquee_speed_label.pack(anchor="w")
        
        self.bright_wave_var = tk.BooleanVar()
        ttk.Checkbutton(self.adv_frame, text="Enable Brightness Wave", variable=self.bright_wave_var, command=self.toggle_bright_wave).pack(anchor="w")
        
        self.bright_wave_frame = ttk.Frame(self.adv_frame)
        ttk.Label(self.bright_wave_frame, text="Wavelength (ft):").pack(pady=(10,0))
        self.wavelength_var = tk.DoubleVar(value=1.0)
        ttk.Scale(self.bright_wave_frame, from_=0.25, to=100, variable=self.wavelength_var, orient=tk.HORIZONTAL, length=300, command=self.update_wavelength_label).pack(pady=5)
        self.wavelength_label = ttk.Label(self.bright_wave_frame, text=f"{self.wavelength_var.get():.2f} ft")
        self.wavelength_label.pack(pady=5)
        
        ttk.Label(self.bright_wave_frame, text="Amplitude:").pack(pady=(10,0))
        self.amplitude_var = tk.DoubleVar(value=0.5)
        ttk.Scale(self.bright_wave_frame, from_=0.1, to=1.0, variable=self.amplitude_var, orient=tk.HORIZONTAL, length=300, command=self.update_amplitude_label).pack(pady=5)
        self.amplitude_label = ttk.Label(self.bright_wave_frame, text=f"{self.amplitude_var.get():.2f}")
        self.amplitude_label.pack(pady=5)
        
        ttk.Label(self.bright_wave_frame, text="Brightness Speed:").pack(pady=(10,0))
        self.brightness_speed_var = tk.DoubleVar(value=0)
        ttk.Scale(self.bright_wave_frame, from_=-100, to=100, variable=self.brightness_speed_var, orient=tk.HORIZONTAL, length=300, command=self.update_brightness_speed_label).pack(pady=5)
        self.brightness_speed_label = ttk.Label(self.bright_wave_frame, text=f"{self.brightness_speed_var.get():.2f}")
        self.brightness_speed_label.pack(pady=5)
        self.bright_wave_frame.pack_forget()
        
        self.mirror_var = tk.BooleanVar()
        ttk.Checkbutton(self.adv_frame, text="Enable Mirror", variable=self.mirror_var, command=self.toggle_mirror).pack(anchor="w")
        
        self.mirror_frame = ttk.Frame(self.adv_frame)
        ttk.Label(self.mirror_frame, text="Mirror Position:").pack(pady=(10,0))
        self.mirror_pos_var = tk.DoubleVar(value=0)
        ttk.Scale(self.mirror_frame, from_=-200, to=200, variable=self.mirror_pos_var, orient=tk.HORIZONTAL, length=300, command=self.update_mirror_pos_label).pack(pady=5)
        self.mirror_pos_label = ttk.Label(self.mirror_frame, text=f"{self.mirror_pos_var.get():.2f}")
        self.mirror_pos_label.pack(pady=5)
        ...
        self.update_btn = ttk.Button(self.bottom_frame, text="Update", command=self.build_command)
        self.update_btn.pack(pady=10)
        
        self.execute_btn = ttk.Button(self.bottom_frame, text="Execute", command=self.execute_marquee)
        ...
        ttk.Label(tab3, text="Placeholder for Tab 3").pack(expand=True)
    ...
    def build_command(self, *args):
        ...
        config["MirrorPosition"] = self.mirror_pos_var.get()
        if self.bright_wave_var.get():
            config["BrightnessWavelength"] = self.wavelength_var.get()
    ...
    def add_color_block(self):
        ...
        self.timeline_row.pack(fill=tk.X, pady=20, before=self.adv_frame)
    ...
    def update_timeline(self):
        ...
        for color, size in self.color_blocks:
            width = max(20, size * 3)
            self.timeline_canvas.create_rectangle(x, 5, x + width, 55, fill=color, outline="black")
            self.timeline_canvas.create_text(x + width / 2, 30, text=f"{size:.2f}ft", fill="white", font=("Arial", 8))
            x += width + 5
        self.timeline_canvas.config(scrollregion=(0, 0, x, 60))
